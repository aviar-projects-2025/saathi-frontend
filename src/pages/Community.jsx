...
export default function Community() {
  ...
  // FIX: was useMediaQuery(theme.breakpoints.down('sm')), which depends on
  // theme.breakpoints being set to the expected values and can report
  // "mobile" on a real desktop window if the theme overrides breakpoints
  // or on the first render before hydration. Now driven directly off
  // window.innerWidth with a resize listener, bucketed into every standard
  // tier (xs/sm/md/lg/xl) so tablets and in-between sizes get their own
  // treatment instead of a single mobile/desktop cutoff.
  ...
  // showSidebar mirrors "desktop/tablet" — sidebar shows whenever there's
  // enough width for it (sm and up), same threshold as before.
  const showSidebar = !isMobile;

  const SAFFRON = "#E8650A";
  const CARD_BORDER = "1px solid #F0E6DC";

  // Post box width now scales smoothly across tiers via the sx prop below
  // (see POST_BOX_WIDTH_SX) instead of a single fixed desktop pixel value,
  // so it looks right on tablets and ultra-wide monitors too, not just one
  // "desktop" bucket.
  const POST_BOX_WIDTH_SX = { xs: '100%', sm: '100%', md: '560px', lg: '600px', xl: '640px' };

  // These still need single JS values (MUI component props like `size` and
  // `fontSize` don't accept responsive objects), so they use the finer tier
  // instead of a single isMobile boolean — phones get the smallest, tablets
  // a middle ground, and laptops/desktops the full size.
...
{/* <Box>// import React, { useEffect, useState, useRef } from 'react';
// import { */}
//   Box, Typography, Grid, Paper, Chip, Avatar, Button,
//   Divider, Stack, LinearProgress,
//   TextField,
